...
實作方式：容器固定為一個鎖住 1440:900 比例的參考舞台（`aspect-ratio: 1440/900`，依視窗比例自動 letterbox，不拉伸），視覺尺寸完全靠 `clip-path: inset()` 控制，內層影片另外走 `transform: scale()`。

**影片本身是 16:9，起點與終點的框都必須鎖在 16:9**，全程比例不變，不會變形、裁切範圍也不會跟著跑：

- 起點（拼貼狀態的小影片框）：left/width 沿用 Figma 實測（node 21:171，46.6667% / 39.9194%），高度改由 16:9 推算（不用 Figma 原始的 355px/39.44%，那不是 16:9），並以 Figma 原框的垂直中心點重新置中，不整組偏移：
  `clip-path: inset(35.2029% 13.4139% 28.8697% 46.6667%)`
- 終點（All Works 區塊的大影片框）：左右各留 40px 邊距（寬度 1360px = 94.4444%，left 2.7778%），高度由 16:9 推算（1360 × 9/16 = 765px = 85%），在視窗內垂直置中：
  `clip-path: inset(7.5% 2.7778% 7.5% 2.7778%)`
...
終點畫面就是一支 16:9 的影片置中，四邊留白，**沒有標題、沒有漸層遮罩、沒有 scroll cue**。這個 progress 區間目前純粹是「影片放大動畫尾段」，沒有額外內容要浮現——之前版本規劃的 `ALL WORKS` 標題與 scroll cue 已經拿掉。
