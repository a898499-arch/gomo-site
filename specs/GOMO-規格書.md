...
Hero 結束後，畫面不是切換，而是一台攝影機持續往前推軌：先推進一片插畫拼貼，再穿過拼貼中央的作品影片，影片一路長大到 All Works 區塊的大尺寸展示框，落在 All Works 的作品列表。

**全程是一個 DOM 結構、一支影片元素。** 影片從插畫中央的小尺寸到 All Works 的大尺寸之間，尺寸永遠連續，沒有任何一刻是重新掛載或交叉淡入。

> ⚠️ 終點不是滿版貼到螢幕邊緣。Figma 的 All Works 區塊影片框（node 3:146）四邊都有邊距，比小尺寸的收斂拼貼狀態大很多、也遠比小框寬，但不是 `inset(0)`。精確數值見下方「Stage B」。

### 二、結構與技術

- 外層 `#practice-to-work` 容器，`height`＝拼貼/放大段落距離＋定住段落距離＋100vh（sticky 舞台自身）。桌機 `180vh + 150vh + 100vh = 430vh`（RWD 各斷點見七、RWD）
- 內層 stage：`position: sticky; top: 0; height: 100vh; overflow: hidden`
- 用 GSAP ScrollTrigger `scrub: 1` 取得原始 `self.progress ∈ [0, 1]`（涵蓋整個容器，含定住段落），再換算成驅動 Stage B 各段動畫的 `p`：
  - `p = clamp01( self.progress × (growth + hold) / growth )`——`growth`＝拼貼/放大段落的 vh 距離（桌機 180），`hold`＝定住段落的 vh 距離（桌機 150）。這個換算讓拼貼散開／標題離場／影片放大這幾段動畫的**手感（每 px 捲動對應的動畫進度）完全不受定住段落影響**，`p` 一旦到 1 就維持在 1，直到使用者往回捲出定住段落才開始遞減

> ⚠️ **2026-08-09 縮短捲動距離**：放大段落從 320vh 縮短到 180vh（總容器高度 570vh → 430vh），使用者要滑更少下才會到 Gallery；接受的取捨是放大過程相對變快。定住段落維持 150vh 不變（刻意保留的觀看時間）。因為上面的 `p` 換算本來就是「放大段落 vh 距離」的比例，Stage B 各段動畫的 `p` 門檻（0.08 / 0.28 / 0.30 / 0.85 等）全部不必更動，縮短只影響每個門檻對應的**絕對捲動距離**，不影響動畫節奏的比例。
  - 對這個 `p` 再做 lerp，factor ≈ 0.08–0.12，製造慣性重量感而非 1:1 生硬跟隨
  - **必須接上 §3.2 的全站 Lenis instance**
- 只操作 `transform` / `opacity`（原規劃用 `clip-path` 控制影片框尺寸，實作後發現這個做法會讓 `object-fit: cover` 誤判裁切基準框，改用「內層媒體框給真實 16:9 尺寸＋`transform: translate()+scale()` 移動縮放」，見四、Stage B）
- **全程 scrubbed**：往上滾必須完整反向，不能有任何只觸發一次的狀態，包含定住段落——退出定住段落、影片縮小、拼貼重新聚攏，都要能順著往回捲直接看到
- **導覽列互動**：`p ≥ 0.3` 起（含整個定住段落）導覽列強制隱藏，覆蓋 §6.1 的「往上滾出現」規則，離開這個範圍才恢復正常行為，見 §4 導覽列行為的例外註記
...
- 每個人形與色塊各自跑慢速 sine：`translateY ±6px`、`rotate ±1.5°`，週期 4–7 秒不等，彼此錯開相位
- 幅度要能感覺到，但**不能搶走影片的視覺焦點**——週期維持緩慢，不要因為幅度加大就跟著加快
...
> 注意起點是 **0.08**——拼貼還在畫面上時影片就已經開始長大，兩段刻意重疊，避免中間出現空白停頓。這裡的 `p` 是換算過的（見二、結構與技術），跟定住段落無關，維持原本的手感。

實作方式：容器固定為一個鎖住 1440:900 比例的參考舞台（`aspect-ratio: 1440/900`，依視窗比例自動 letterbox，不拉伸）。

> ⚠️ **實作教訓（原規劃是 `clip-path: inset()`，已改掉）**：把影片元素固定在 100% 大小、只用 `clip-path` 裁出可視窗口，這個做法有個隱藏 bug——`object-fit: cover` 是依元素「自己的 layout 尺寸」決定怎麼裁切內容，`clip-path` 只影響畫面上看不看得到，完全不影響 layout 尺寸。結果內層影片永遠是照著外層那個固定的 1440:900（16:10）框在做 cover 裁切，跟畫面上實際看到的 16:9 視窗對不上，導致內容邊緣被裁掉一截（例如佔位圖左側文字被切掉）。
> **正確做法**：內層媒體框本身就給它真實的 16:9 layout 尺寸（`width:100%; aspect-ratio:16/9`），用 `transform: translate() scale()` 把這個框移動、縮放到目前該在的位置與大小（`transform-origin:0 0`，縮放比例＝當下寬度 ÷ 基準寬度）。`object-fit: cover` 這時候是依照真正的 16:9 框在裁切，內容才會完整、跟畫面看到的窗口一致。圓角兩端都是 0（下方有說明），所以不需要 `clip-path` 做任何事——整段全部用 `transform`，比原規劃更貼合「只動 transform/opacity」的效能原則。

**影片本身是 16:9，起點與終點的框都必須鎖在 16:9**，全程比例不變，不會變形、裁切範圍也不會跟著跑（起點終點的「% of 1440」寬與「% of 900」高比例都恰好是 10:9，線性插值全程維持這個比例，換算成像素就是全程精確 16:9，不只頭尾兩點對）：

- 起點（拼貼狀態的小影片框）：left/width 沿用 Figma 實測（node 21:171，46.6667% / 39.9194%），高度（=寬度×9/16）改由 16:9 推算（不用 Figma 原始的 355px/39.44%，那不是 16:9），並以 Figma 原框的垂直中心點重新置中，不整組偏移：`left 46.6667%、top 35.2029%、width 39.9194%`
- 終點（All Works 區塊的大影片框）：左右各留 40px 邊距（寬度 1360px = 94.4444%，left 2.7778%），高度由 16:9 推算（1360 × 9/16 = 765px = 85%），在視窗內垂直置中後**再往下移 20px**（2.2222%，2026-08-08 回饋）：`left 2.7778%、top 9.7222%、width 94.4444%`
- 圓角：兩個 Figma 背景矩形（Rectangle 80 / Rectangle 60）原始 SVG 都是 `<rect>` 沒有 `rx`，起點與終點圓角都是 **0**
- 內層 `<video>`：`scale(1.12 → 1.0)`，反向推近的視差感
- 陰影：`0 24px 60px rgba(0,0,0,0.18)` → 完全消失
- **放大曲線要慢、穩、有份量：前段加速、後段長長地減速。是推軌，不是 zoom**

**p = 0.85 → 1.0：定住（2026-08-08 新增）**

影片放大完成後不立刻結束——維持在最大尺寸不再變化，讓使用者有時間好好看內容，捲完這段才解除 sticky、繼續往下到 Gallery。

- 桌機定住距離 150vh，平板/手機依各自放大段落距離等比例縮小（見七、RWD）
- 這段期間：**影片持續播放、持續輪播**（這正是定住的目的）；**導覽列維持強制隱藏**（見二、結構與技術）；**沒有其他元素出現**——沒有標題、沒有漸層遮罩、沒有 scroll cue，畫面就是一支置中的 16:9 影片
- 全程 scrubbed：往上滾完整反向——先退出定住區、影片開始縮小、拼貼重新聚攏，不能有卡住的狀態
...
- **相鄰動畫區間必須重疊**（拼貼還沒散完影片就開始長、影片還沒長到 All Works 大尺寸 All Works 標題就開始浮現），任何一處出現空檔都會讓推軌感斷掉
- 不要彈性、不要 overshoot、不要 bounce

### 七、RWD

| 斷點 | 影片起始寬 | 插畫 | 放大段落 | 定住段落 | sticky 容器高（放大＋定住＋100vh）|
|---|---|---|---|---|---|
| 桌機 | 40vw | 五個人形 + 全部色塊 | 180vh | 150vh | 430vh |
| 平板 | 55vw | 減為三個最主要的人形 + 色塊 | 146vh | 120vh | 366vh |
| 手機 | 80vw | 只留兩個人形，改放在影片上下而非環繞 | 90vh | 75vh | 265vh |

放大段落：平板/手機＝桌機放大段落 × (180/320)，依此比例縮放並四捨五入到整數 vh。定住段落維持絕對值不變（120vh / 75vh，2026-08-09 之前就已是這兩個值），不隨放大段落縮放。
...
2. Idle 漂移感覺得到、但不會搶走影片的視覺焦點
3. 影片變大時，插畫是向外放射散開並淡出，**不是被蓋住**
4. 前層與後層插畫的移動速度**肉眼可辨地不同**
5. 輪播在整個放大過程持續換片，不卡頓
6. 影片從插畫中央的小尺寸到 All Works 的大尺寸**全程尺寸連續**——在任一幀暫停都看得到中間尺寸，不會跳格
7. Hero → 拼貼 → All Work 之間沒有任何硬切換、閃白、或背景色階梯
8. 往上滾完整反向，沒有卡住的狀態
9. 放大全程影片不變形、不出現黑邊
10. 影片放大到最大尺寸後維持在該尺寸一段可感覺到的時間（定住段落），不會放大完立刻捲出區塊
11. 定住段落期間：影片持續播放/輪播、導覽列維持隱藏、沒有其他元素出現
12. 導覽列在 p ≥ 0.3（含定住段落）強制隱藏，往上滾也不會叫出來；離開這個範圍後恢復正常 hide-on-scroll
13. 中階筆電上放大階段維持 60fps
14. 手機上不會因為 sticky 導致滾動卡頓或高度計算錯誤

### 十、捲動到下一區塊按鈕（首頁專用，2026-08-09 新增）

首頁捲動距離長，加一顆固定按鈕讓使用者可以一鍵跳到下一個有意義的停留點，不用手動滑好幾下。

**外觀與位置**

- **共用 §6.4 Footer 的 back-to-top 圓形按鈕元件（同一個 CSS class）**，不另外做一個獨立元件——之後改樣式兩邊要一起變。箭頭方向改為朝下（視覺上把共用的上箭頭圖示整個轉 180°，不是另外畫一份 SVG）
- 尺寸、hover/focus-visible 效果（描邊加粗、填入 accent 紅、箭頭轉白並位移、放大到 1.06，400ms）與 back-to-top 完全一致，因為是同一個 class
- `position: fixed`，畫面右側垂直置中；距離右邊界的距離與 footer 那顆的右邊距一致（`.footer-frame` 的右 padding）
- `z-index` 低於導覽列（導覽列 1000 恆為最上層），但高於頁面內容
...
- Loading 動畫期間不顯示
- 捲到 Footer（已經是最後一段，沒有「下一個」了）時淡出隱藏；往回滾離開 Footer 會再淡入
- 淡入淡出 300ms

**可存取性**

- 真正的 `<button>`，可鍵盤聚焦，`aria-label="Jump to next section"`
- `:focus-visible`：1px accent 紅描邊，4px offset（與 back-to-top 共用同一條規則，見 §6.4）
- `@media (prefers-reduced-motion: reduce)`：捲動 duration 縮短為 300ms
...
區塊標題：`UIUX/ PRODUCT DESIGN/ APP DESIGN / CAD/ CRAFT/ REFLECTIVE DESIGN +MORE`（紅色、大寫、兩行，Poppins **SemiBold 600**、40px、`#c90000`、tracking -0.2px、line-height 1.45，位置在 Gallery 那排卡片正上方 175px）

> ⚠️ 字重 2026-08-08 改為 600——Figma node 3:148 原始字重是 Medium 500，這裡是使用者明確要求的偏離值（「目前太粗，改成 600，不要 700 或 800」），不是 Figma 誤讀，特此註明兩者不同的原因。

> 本節 2026-08-08 做過一輪修訂（見下方各段落的版本記錄），修正三處跟原規格互相矛盾或缺漏的地方：Layer 2 把垂直滾輪轉成水平位移會讓使用者永遠捲不到 footer；「落定對齊」跟 Layer 1 的持續漂移在數學上不可能同時成立；卡片尺寸的 40vw/6vw 跟「sliver」的視覺描述自相矛盾。

### 整體體驗

慢、優雅、電影感、奢華。每個動作都有重量與慣性。**不俏皮、不彈跳、不彈性。** 那種感覺是在現代設計博物館裡瀏覽作品——動效存在的唯一目的是強化工藝感的知覺，絕不引起對自身的注意。

**區塊標題進場**：與全站一致的 masked line reveal——兩行文字各自包在 `overflow:hidden` 容器內，`translateY(100%) → 0`，750ms，主曲線，第二行延遲 130ms。文字本身不淡入，只靠遮罩。`ScrollTrigger` 在區塊捲入視窗時觸發一次（`once: true`），不隨 Layer 1/2 的水平位移重播。

### 版面

> ⚠️ 2026-08-08 修訂：以下尺寸改為 Figma 實測值（node 3:150～154，Gallery 那排灰卡片），取代原規格的 40vw/6vw——那組數字算出來卡片前後各露出 24vw，跟「sliver」的描述自相矛盾。

- Gallery**不 pin、不 sticky**，是頁面上一個正常高度的區塊（原規格暗示的滾輪轉軌道會讓使用者永遠捲不到 footer，已拿掉，見下方「動效系統」修訂一）
- 作品排成一條連續的水平軌道
- 每張卡片 **28.63vw 寬**（Figma 412.252px / 1440），高度用 `aspect-ratio: 412.252 / 555 ≈ 0.7428` 隨寬度推算（不是獨立設一個 vh 值，避免兩個獨立設定在非 16:9-ish 視窗比例下互相打架），卡片間距 **固定 24px**（2026-08-08 改為固定 px，不用 vw——原本 Figma 換算出來的 1.95vw 在使用者測試時感覺太寬）
- 用實際數字試算 1440 寬視窗：置中卡片兩側各剩 513.87px，扣掉間距後約再露出 1.18 張卡片——使用者實際看到的是「**1 張置中主視覺 + 左右各接近完整的 1 張 + 一點點第 3 張的邊**」，不是原規格講的「置中 1 張 + 前後 sliver」
- 無邊框。必要時才用極淡的陰影。大量留白

### 動效系統——三層，優先順序明確

有三個獨立的水平位移來源。**必須照這個優先順序實作**：

**Layer 1 — 環境自動捲動（基線）**

- 軌道持續以固定速度向左漂移（約 **50px/秒**，2026-08-08 從 18–25px/秒調高——要明顯看得出在移動，但仍然是從容的展示速度，不是跑馬燈）。它要讀成「活著」，不是幻燈片，作用是暗示「這裡可以滑」
- **無縫無限循環**：軌道兩側（前後）各複製一份，對軌道寬度取模 `translateX`，讓接點永遠在視窗外，不能看到跳接或終點；兩側都複製而不是只接一份，是因為只接一份會讓「置中第一張／最後一張卡片」這類邊界情境找不到內容可以 wrap，鍵盤置中時會落在複製品身上而不是真正拿到 focus 的那個元素
- ⚠️ 2026-08-08 修訂（暫停條件，取代前一版「互動過就永久停掉」）：**只有以下兩種情況才暫停**——(a) 使用者按住拖曳期間，(b) 觸控滑動期間（含觸控板水平手勢，視為同一類）。放開後 **800ms** 緩緩恢復到原速度。**單純把游標移過去（hover）不算互動，不會暫停軌道**——hover 卡片本身的視覺效果（見下方 Hover 互動）照常觸發，只是不影響軌道移動

**Layer 2 — 使用者手動位移（覆蓋 Layer 1）**

> ⚠️ 2026-08-08 修訂：原規格「垂直滾輪 / 觸控板捲動轉換為軌道水平位移」拿掉——這會讓使用者往下滾時卡在原地，永遠到不了 footer。這個區塊的垂直捲動永遠是正常翻頁行為。

水平位移只來自：

1. 桌機滑鼠按住拖曳（`pointerdown` + `pointermove`）
2. 觸控裝置水平滑動、觸控板水平滑動手勢

用 **Lenis**（§3.2 的同一個 instance，但只用於偵測捲動速度以判斷手勢方向，不用於實際位移軌道）搭配自訂的重慣性平滑，手感應該像在房間裡推動一面沉重的展牆。

**方向判斷（避免跟垂直捲頁衝突，手機上尤其重要）**：手勢開始時比較垂直分量與水平分量——

- 垂直分量較大 → 判定為「使用者想捲頁面」，放行給瀏覽器原生捲動，**不攔截**，也不觸發任何軌道位移
- 水平分量明顯較大 → 判定為軌道手勢，`preventDefault` 攔截，接管為軌道位移

**落定行為**：⚠️ 2026-08-08 修訂——原規格「速度低於門檻時滑行對齊最近卡片」整段拿掉。理由：這個行為在數學上跟 Layer 1 的持續漂移互斥（對齊完成後漂移一恢復，卡片馬上又飄走），兩者不可能同時成立，而且 Layer 1 現在也不會恢復（見上）。改為：**軌道自由停在放手時的任意位置，不做任何強制對齊**。放手後只有自然的慣性減速，減速結束就停在哪算哪。
...
- **其他所有卡片**：基礎 scale 乘 **0.96**，opacity 降到 **0.45**，並套上 grayscale
- ⚠️ 2026-08-08 修訂（兩個 filter 疊加規則，原規格沒說）：saturate（Layer 3 的距中聚焦）與 grayscale（hover 的其他卡片）**寫在同一個 `filter` 宣告裡，順序固定 `filter: saturate(X%) grayscale(Y%)`**，先飽和度後灰階。非 hover 狀態 grayscale 恆為 0%；hover 時其他卡片的 grayscale 才到 100%。兩個值各自獨立過場：**saturate 400ms，grayscale 500ms**（grayscale 略慢，讀成氛圍而非狀態切換，scale/opacity 也是 400ms 跟 saturate 同步）
- 被 hover 的卡片上淡入 overlay，向上錯開各 40ms：**作品標題 / 分類 tags**（年份、一句話描述待補——見下方原型階段暫行處理，版面已預留位置）
...
> ⚠️ **原型階段暫行處理**（詳情頁還沒做）：`href` 寫真實路徑（`/work/[slug]`，`slug` 取自 §5.1），可右鍵開新分頁、可鍵盤聚焦，但點擊時 `preventDefault()` 擋住跳轉。shared-element transition 這次不做。之後詳情頁完成，只要移除 `preventDefault` 就會通，不用重寫連結結構。

### 排版與視覺

大尺寸 editorial 排版，瑞士風格，現代無襯線，強烈層級，極少文字。柔和中性配色。極簡博物館美學。**圖片始終是主角。** Gallery 中每件作品只放一張大主圖——輔助圖片只出現在詳情頁。

> ⚠️ **原型階段暫行處理**：作品照片還沒有，卡片內容先用 `#d9d9d9` 灰色色塊代替（跟 Figma 稿上那排灰格子一致），尺寸/比例照上面「版面」段落的實測值。之後換成真實照片時不需要改版面。
...
- ⚠️ 2026-08-08 修訂（無限循環的無障礙處理，原規格沒提）：為無縫循環複製出來的那一份軌道，容器加 `aria-hidden="true"`，內部**所有**可聚焦元素（`<a>`）都設 `tabindex="-1"`。只有原始那一份參與鍵盤導航（Tab／方向鍵）與螢幕閱讀器的無障礙樹，避免 Tab 走到複製品、或螢幕閱讀器讀出 24 件重複作品

### 效能

- 只動 transform 與 opacity（filter 見上方 hover 段落）。**絕不動 layout 屬性**（width、height、left、top、margin）
...
1. 軌道載入後自行漂移，僅在拖曳／觸控滑動期間優雅停下，放開後 800ms 緩緩恢復；單純 hover 不影響漂移
2. hover 任一卡片會讓其他所有卡片變灰縮小，**無閃爍、無掉幀**
3. hover 狀態乾淨地覆蓋距離狀態——兩套 scale 系統之間沒有可見的「打架」
4. 拖曳/滑動放手後只有自然慣性減速，**不做任何強制對齊**，停在哪算哪
5. 循環**沒有可見接縫**，複製軌道對鍵盤導航與螢幕閱讀器不可見
6. 垂直捲頁永遠正常運作，不會被水平軌道手勢攔截；反之水平手勢也不會誤觸發垂直捲動
7. 點擊卡片透過真實 URL 導航（原型階段 `preventDefault`，見上）
8. 區塊標題進場只播一次，跟 Layer 1/2 的水平位移互不影響
9. 中階筆電載入 12 件以上作品時持續 60fps
...
> **2026-08-11 對齊基準**：本頁簡介文字、篩選頁籤、卡片格線的左邊界統一使用全站共用的 `--page-gutter`。Figma 稿上這三者左緣本身就不一致（簡介/頁籤在 x=50，卡片格線在 x=38），視為設計稿本身的誤差，不特別去對齊 Figma 的任一個數字——全站左緣一致比複製 Figma 的不一致更重要。
