...
實作方式：容器固定為一個鎖住 1440:900 比例的參考舞台（`aspect-ratio: 1440/900`，依視窗比例自動 letterbox，不拉伸），視覺尺寸完全靠 `clip-path: inset()` 控制，內層影片另外走 `transform: scale()`。`clip-path` 的四個 inset 值都是這個 1440×900 參考舞台裡兩個 Figma frame 的實測邊距換算來的，不是對稱猜測：

- 起點（拼貼狀態的小影片框，node 21:171，574.84×355 置中於拼貼群組內）：
  `clip-path: inset(33.4444% 13.4139% 27.1111% 46.6667%)`
- 終點（All Works 區塊的大影片框，node 3:146，1381×682，四邊都留白，**不是**滿版貼邊）：
  `clip-path: inset(21.8889% 2.7778% 2.3333% 1.3194%)`
- 圓角：兩個 Figma 背景矩形（Rectangle 80 / Rectangle 60）原始 SVG 都是 `<rect>` 沒有 `rx`，起點與終點圓角都是 **0**，不需要 round 項、也不需要圓角收斂動畫
...
沒有另外一組作品 grid——長大到 All Works 大尺寸的影片本身（輪播播放中的作品）就是「All Works」的內容展示：

- 區塊標題 `ALL WORKS`（accent 紅、底線，Figma node 3:143：Poppins Medium 40px、uppercase、`#c90000`、tracking -0.2px）從 `translateY(80px), opacity 0` 浮現到定位。**位置在影片框正上方的空白區，不是疊在影片上**（Figma 標題 y=1922，影片框 y=1997，中間 17px 純背景間距）
- 影片上疊一層由下往上的漸層遮罩，`opacity 0 → 0.7`，供疊加在影片邊角上的次要 UI（如 scroll cue）維持可讀性
- 出現 scroll cue（細線 + `Scroll to explore`）——Figma 無此元素的座標參考，位置為實作自行決定
