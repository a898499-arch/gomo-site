...
區塊標題：`UIUX/ PRODUCT DESIGN/ APP DESIGN / CAD/ CRAFT/ REFLECTIVE DESIGN +MORE`（紅色、大寫、兩行，Poppins Medium 40px、`#c90000`、tracking -0.2px、line-height 1.45——Figma node 3:148 實測，位置在 Gallery 那排卡片正上方 175px）
...
- 作品排成一條連續的水平軌道
- 每張卡片 **28.63vw 寬**（Figma 412.252px / 1440），高度用 `aspect-ratio: 412.252 / 555 ≈ 0.7428` 隨寬度推算（不是獨立設一個 vh 值，避免兩個獨立設定在非 16:9-ish 視窗比例下互相打架），卡片間距 **1.95vw**（Figma 28.14px／1440，取前三個一致的間距，第四個 33.768px 是 Figma 排版離群值，不採用）
...
- 軌道持續以極慢的固定速度向左漂移（約 **18–25px/秒**）。它要讀成「活著」，不是幻燈片，作用是暗示「這裡可以滑」
- **無縫無限循環**：複製軌道，並對軌道寬度取模 `translateX`，讓接點永遠在視窗外，不能看到跳接或終點
- ⚠️ 2026-08-08 修訂（暫停後不再恢復）：一旦使用者主動互動過 gallery（游標進入、拖曳、觸控滑動任一項），自動漂移在約 600ms 內緩緩停下，**且不再恢復**，之後軌道完全交給使用者控制。原規格「閒置 2.5 秒後恢復」已拿掉——自動漂移跟使用者控制清楚二分，永遠不會互相搶奪狀態
...
1. 軌道載入後自行漂移，使用者一旦主動互動過就優雅停下，且不再自行恢復
