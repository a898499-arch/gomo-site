...
2. **絕不動 `width` / `height` / `top` / `left` / `margin`**
   （唯一例外：§6.1 職位輪播容器寬度，需用 ResizeObserver 測量後以 transform 實現）
...
### PHASE 1 — Loading 組裝（0 → 1.4s）

滿版 `--bg` 畫布，所有元素置中。**嚴格照順序出現，不可同時**。

**1.1 兩張板凳落地（0 → 700ms）**

兩張紅色板凳 SVG 並排於視窗正中央，中間留一道窄縫（logo 之後會放這）。此時尺寸小，各約 **90px 寬**。
...
**1.2 GOMO logo 出現（700 → 1150ms）**

字標 logo（互扣的圓形字體，墨黑）在兩張板凳之間淡入，板凳輕輕讓開：

- Logo：`scale(0.85) opacity 0` → `scale(1) opacity 1`，450ms
- **同時**兩張板凳各向外水平滑開約 24px 拉開間隙。相同 450ms、相同 easing
- 讓開與 logo 到位必須讀成**同一個動作**，不是兩件事

**1.3 計數器出現（1150 → 1400ms）**

百分比文字在 logo 正下方淡入，起始 `0%`。淡入 + 上升 8px，250ms。**先不開始計數**，在 0% 停一拍。

### PHASE 2 — 計數器（1.4s → 載入完成，最短 2.5s）
...
**進度來源。** 追蹤真實資產載入（Hero 圖片、字體、板凳 SVG），但**強制最短 2.5 秒**。
...
- **放大**：約 90px 寬 → Hero 全尺寸（約 3 倍）。成長曲線**前段加速**，衝出 loading 狀態後**減速落定**——就是鏡頭推進然後穩住
- **分開**：兩張板凳水平分開到 Hero 的較寬間距
- **下降**：從視窗正中央往下移到 Hero 的中下位置

Duration 1300ms，主曲線。**左板凳領先，右板凳晚 80ms**，不要像一整塊剛體在動。

> **關鍵**：板凳在任何一刻都不可看起來「切換」到新尺寸。1300ms 全程縮放必須連續。SVG 要保持向量，3 倍放大才會銳利——**不可用點陣圖**。
...
| 職位 | Poppins 700、`--accent`、**全大寫**、字距收緊 | `INDUSTRIAL DESIGNER` 等 |
...
**版面規則——先解這題。** 三組短語寬度差很多（`MAKER` 比 `INDUSTRIAL DESIGNER` 短非常多）。因為這一行是**置中**的，如果不處理，整行會在每次替換時左右跳動。

實作要求：

- 整行（冠詞 + 職位）當作**一個置中的單位**
- 容器寬度以 **ResizeObserver 量測**目標短語寬度，用 400ms 主曲線平滑過渡到新寬度
- **寬度變化必須用 transform 實現，不可直接動 `width` 觸發 layout**
  （做法：外層固定最寬尺寸，內層用 `transform: scaleX()` 搭配 clip，或量測後以 translateX 補償兩側）
- 保留一行的 `min-height`，下方內容永不位移
- 冠詞與職位之間的空格寬度固定，不隨字寬變化

**替換動畫。** 在 `overflow: hidden` 包裝內：

- 舊短語：`translateY(0 → -100%)`、`opacity 1 → 0`，450ms
- 新短語：`translateY(100% → 0)`、`opacity 0 → 1`，450ms，在舊的開始後 80ms 才啟動
- 主曲線
- **冠詞與職位一起換，同一個動作**，不要分開

**時間。** 每組停留 2000ms 後替換。無限循環：Industrial → Product → Maker → Industrial → …
...
| 手機 | 板凳縮到 88vw 內、間距靠近；**對話框改放在各自板凳的上方**而非旁邊，避免被視窗邊緣裁切；推軌縮短為 1000ms（小螢幕上長轉場感覺更慢）；職位停留仍為 2000ms |
...
- 完全跳過 loading 組裝與推軌
- 顯示靜態 loading 狀態，**計數器仍然計數**（計數是資訊，不是裝飾），但改為固定線性速度
