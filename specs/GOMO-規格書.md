...
- 外層 `#practice-to-work` 容器，`height`＝拼貼/放大段落距離＋定住段落距離＋100vh（sticky 舞台自身）。桌機 `320vh + 150vh + 100vh = 570vh`（RWD 各斷點見七、RWD）
...
- 用 GSAP ScrollTrigger `scrub: 1` 取得原始 `self.progress ∈ [0, 1]`（涵蓋整個容器，含定住段落），再換算成驅動 Stage B 各段動畫的 `p`：
  - `p = clamp01( self.progress × (growth + hold) / growth )`——`growth`＝拼貼/放大段落的 vh 距離（桌機 320），`hold`＝定住段落的 vh 距離（桌機 150）。這個換算讓拼貼散開／標題離場／影片放大這幾段動畫的**手感（每 px 捲動對應的動畫進度）完全不受定住段落影響**，`p` 一旦到 1 就維持在 1，直到使用者往回捲出定住段落才開始遞減
...
|---|---|---|---|---|---|
| 桌機 | 40vw | 五個人形 + 全部色塊 | 320vh | 150vh | 570vh |
| 平板 | 55vw | 減為三個最主要的人形 + 色塊 | 260vh | 120vh | 480vh |
| 手機 | 80vw | 只留兩個人形，改放在影片上下而非環繞 | 160vh | 75vh | 335vh |

定住段落＝放大段落 × (150/320)，依各斷點自己的放大距離等比例縮放，四捨五入到整數 vh。
