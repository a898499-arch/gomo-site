...
// 那三個獨立元素，不再各自實作。
//
// ⚠️ 尺寸還是對不上，第二次回報：你先說「1244×910」，這輪改口
// 「1244×810」，但實際讀檔（sharp 量出來）一直是 2888×1738px，比例
// 1.6617，跟兩個數字都對不上（1244:910=1.367、1244:810=1.536）。重讀
// Figma 545:510 發現「810」很可能是手機遮罩本身的 mask-size
// （468.686×810，見 744:71 那個 node），不是整張合成圖的高度——合成圖
// 真正的組合外框（紅色漸層 744:68 + 手機遮罩 744:71 兩者的聯集）我自己
// 算出來大約是 1244×820，用你的兩個數字都對不上，只有拿檔案自己的比例
// （1244×1738/2888＝748.6）才能保證這張「不該再被裁切」的圖不會又被
// object-fit:cover 硬套一個不合的框、重新做出你這輪想解決的裁切問題。
// 這裡繼續用實測的 748.6，等你確認要不要換一份精確尺寸的匯出檔。
//
// 這輪順便修正合成圖的定位錨點：重讀 Figma 545:510 發現圖層座標系換過
// 一次——外層分組 node 744:67（包住紅色漸層 744:68 跟手機遮罩 744:69/71
// 兩者）的絕對座標是 left:163,top:47，不是我上一輪沿用的舊座標
// left:263,top:51（那組數字是合成圖出現以前、紅色漸層自己單獨的舊座標，
// 已經過時）。改用 (163,47) 當合成圖左上角。
//
// 整體用「100vw 出血」做法：
...
//   合成圖（紅色漸層+右手機+
//   底部淡出遮罩）：         x=163 y=47  w=1244 h=748.6（見上方尺寸註記）
...
//     ——重讀 Figma 545:527 絕對座標 x=1051,y=13 沒變，減掉合成圖新錨點
//     (163,47)：1051-163=888，13-47=-34。
//
...
// 合成圖實際檔案比例反推的高度，見檔頭的尺寸不一致說明。
const COMPOSITE_W = 1244;
const COMPOSITE_NATIVE_RATIO = 1738 / 2888; // 量檔案量到的，不是猜的
const COMPOSITE_H = COMPOSITE_W * COMPOSITE_NATIVE_RATIO; // ≈748.6
...
// 不再是分開的紅色漸層 + 手機 + 遮罩三層。
const COMPOSITE = {
  x: 163, y: 47, w: COMPOSITE_W, h: COMPOSITE_H,
...
export default function Function2() {
  ...
  // wrapper 該多高——目前是左手機（349+490=839），比合成圖（51+748.6≈
  // 799.6）跟中手機（235+590=825）都高。子元素全部 position:absolute，
  // wrapper 自己沒有這個高度就會塌成 0，把後面的區塊往上蓋過來。
  const contentH = 839;
