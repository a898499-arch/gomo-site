...
- 目前已套用：Sui-Sui、Goodmood、eHMS、WanderBuddy、AERO V（Overview 與手機版；桌機各區是 Figma 構圖等比縮放，字級跟著框寬走）。其他頁面逐頁套用，套用時一樣一次一到兩個區塊。
...
- 目前已套用：eHMS（原本就是）、Goodmood、Sui-Sui（手機畫面輪播維持滿版，Maida 指定）、WanderBuddy（Hero、操作動畫 FlowAnimation、Sign Up Flow、角色漂移、8 Activity／8 Characters 參考圖、四支手機主圖維持滿版，Maida 指定）、AERO V（Hero 維持滿版）。
